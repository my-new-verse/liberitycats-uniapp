    ...
    plugins: [
      ...
      }),
      {
        ...
        configureServer(server) {
          // 删除/dist/dev/app目录
          const distPath = path.resolve(__dirname, 'dist/dev/app')
      ...
      UNI_PLATFORM === 'app' && copyNativeRes(),
      {
        name: 'build-info',
        writeBundle() {
          // const version = process.env.VERSION || '1.0.0' // 从环境变量中获取版本号
          // const buildTime = new Date().toISOString() // 获取当前时间
    ...
    server: {
      ...
      proxy: JSON.parse(VITE_APP_PROXY)
        ? {
            ...
            '/api2': {
              target: 'https://stripe-mobile-payment-sheet.glitch.me',
              changeOrigin: true,
