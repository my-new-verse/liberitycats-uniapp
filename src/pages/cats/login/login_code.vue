...
import { sendEmailApi, sendEmailResponse, LoginResponse } from '@/service/api/login'
...
const doLogin = async () => {
  try {
    ...
    const { loginByEmailCode, navigateToAfterLogin } = useUserStore()
    const res = await loginByEmailCode(data.email, code.value)
    // 登录成功，执行跳转
