...
import { loginApi, sendBindVirtualEmailApi } from '@/service/api/login'
...
const sendEmail = () => {
  ...
  // 查询虚拟账号是否存在
  sendBindVirtualEmailApi(inputEmail.value).then((res) => {
    if (res.code === 1) {
      uni.redirectTo({
        url:
          '/pages/cats/login/virtual/bind_code?email=' +
          inputEmail.value +
          '&agreement=' +
