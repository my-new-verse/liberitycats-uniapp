...
function handleClick(event: MouseEvent) {
  if (props.enablePreview && props.src && status.value === 'success') {
    const urls =
      Array.isArray(props.previewSrc) && props.previewSrc.length > 0
        ? props.previewSrc
        : props.src
          ? [props.src]
          : []
    if (urls.length > 0) {
      // 移除当前图片的查询参数
      const currentSrc = props.src.split('?')[0]

      // 移除所有预览图片的查询参数
      const cleanUrls = urls.map((url) => url.split('?')[0])

      // 查找当前图片在预览列表中的索引
      const current = cleanUrls.indexOf(currentSrc)

      console.log('======================cleanUrls', cleanUrls, current)
      uni.previewImage({
        urls: cleanUrls,
